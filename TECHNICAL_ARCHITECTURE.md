...
- **Head:** a small trainable projection (`ProjectionHead`: Linear → ReLU → Dropout → Linear, ~230K parameters, L2-normalized output) sitting on top of the frozen backbone's features, trained with a triplet-margin loss on the labeled anomaly library (`anomaly_type`/`identification` as supervision). Weights are stored at `LIBRARY_DATA_DIR/models/dinov2_base_head_v2.pt` (`Settings.metric_head_path`) — data, not code, so it lives alongside `metadata.json` and gets the same Dropbox-backed versioning for free (see §7.1).
- **Training data — every panel image, not just the primary, but never a same-anomaly positive pair** (`scripts/train_metric_head.py`, `scripts/evaluate.py`'s `load_library_corpus`): an anomaly's additional panel images (`Image.additional_image_paths`) are real, valid training examples of its class, not near-duplicates of the primary — confirmed against the actual corpus, most anomalies' stored images span genuinely different panel types (Beamforming vs. Image vs. Raw), and even same-panel-type duplicates mostly turned out to carry a different `beamforming_types` sub-mode. Because two images of one anomaly can look substantially different from each other, the triplet loss's positive pairs are **never** drawn from the anchor's own anomaly — a positive always comes from a genuinely different anomaly that happens to share the same label — and K-fold cross-validation splits by anomaly, not by individual image, so a held-out query's own sibling panels are never in the training set that produced the head scoring it. (v1, superseded, trained on primary images only and didn't need this rule since there were no same-anomaly pairs to encounter at all; v2 measurably beat it on the same corrected protocol — see `model_registry.json`'s `deployment` entries.)
...
| `metric_head_path` | `models/dinov2_base_head_v2.pt` | Trained head weights, relative to `library_data_dir` |
...
- **Model Tag** — a string identifying exactly which backbone+head (or CLIP model+checkpoint, for the legacy rerank model) produced a given embedding (e.g. `vit_base_patch14_dinov2.lvd142m+dinov2_base_head_v2`), used to prevent comparing incompatible vector spaces.
