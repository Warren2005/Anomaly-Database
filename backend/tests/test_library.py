...
class TestUpdateLibraryEntryConcurrency:
    """Guards against two people editing (or one deleting while another
    edits) the same entry — see the not-found rewording above and the new
    expected_updated_at check below."""

    def test_update_with_matching_expected_updated_at_succeeds(self):
        image_id = uuid4()
    ...
    def test_update_with_stale_expected_updated_at_is_conflict(self):
        """Someone else's edit changed updated_at since this client loaded
        the entry — reject instead of silently overwriting their change."""
        image_id = uuid4()
        existing = _make_image(image_id)
        with patch("app.api.v1.endpoints.library.file_store_service") as mock_store:
            mock_store.get_image = AsyncMock(return_value=existing)
    ...
    def test_update_without_expected_updated_at_skips_the_check(self):
        """Optional — direct API callers that don't track it aren't broken."""
        image_id = uuid4()
        existing = _make_image(image_id)
        raw_record = {
            "embedding": [0.1], "rerank_embedding": None,
    ...
    def test_update_deleted_entry_gives_a_clear_not_found_message(self):
        image_id = uuid4()
        with patch("app.api.v1.endpoints.library.file_store_service") as mock_store:
            mock_store.get_image = AsyncMock(return_value=None)
            ...
            assert response.status_code == 404
            assert "deleted by someone else" in response.json()["error"]["message"]
